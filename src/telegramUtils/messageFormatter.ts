...
export function formatMessageAllMonths(entries: Saving[]): string {
	const grouped = entries
		.filter(
			(entry) => typeof entry.amount === 'number' && entry.description.trim()
		)
		.map((entry) => {
...
export function formatMessageByMonth(month: string, entries: Saving[]): string {
	let total = 0;
