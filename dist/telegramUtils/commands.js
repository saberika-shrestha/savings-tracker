import { getAllSavings, getSavingsByMonth, deleteSavingsByMonth, } from '../database/database.js';
import { sendMessage } from './messageSender.js';
import { formatMessageAllMonths, formatMessageByMonth, } from './messageFormatter.js';
import { monthAbbrevToNumber } from '../types/savings.js';
export function handleCommands(chatId, text) {
...
function listSavingsMonth(text) {
    ...
    if (month.length > 3)
        return 'Please provide a month in short format\\.';
    if (!/^[a-zA-Z]+$/.test(month))
        return 'Please provide a valid month\\.';
    const monthNumber = monthAbbrevToNumber[month];
    const entries = getSavingsByMonth.all({ month: monthNumber });
    const formatted = formatMessageByMonth(monthNumber, entries);
    return `${formatted}`;
}
function removeSavingsByMonth(text) {
    const month = text.split(' ')[1];
    if (!month)
        return 'Please provide a month\\.';
    if (month.length < 3)
